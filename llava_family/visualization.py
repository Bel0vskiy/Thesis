...
def save_perturbation_curve(
    ...
    *results_by_method* maps ``method_name`` → evaluation result dict
    ...
    for i, (method, res) in enumerate(results_by_method.items()):
        drops = {float(k): v for k, v in res.get("mean_drops_by_ratio", {}).items()}
        if not drops:
            continue
...
def save_aggregate_curves(
    ...
    for i, (method, sample_results) in enumerate(all_results.items()):
        ...
        for sr in sample_results:
            all_ratios.update(float(k) for k in sr.get("mean_drops_by_ratio", {}).keys())
        ratios = sorted(all_ratios)
        means = []
        stds = []
        for r in ratios:
            vals = [
                sr["mean_drops_by_ratio"].get(r, sr["mean_drops_by_ratio"].get(str(r)))
                for sr in sample_results
                if r in sr.get("mean_drops_by_ratio", {}) or str(r) in sr.get("mean_drops_by_ratio", {})
            ]
        ...
        aopc_vals = [sr.get("aopc", 0) for sr in sample_results]
